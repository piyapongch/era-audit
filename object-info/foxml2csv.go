...
func main() {
	...
	csv = append(csv, "DCQ MD5")
	csv = append(csv, "DS MD5")
	...
	for i, n := range xmlquery.Find(doc, "/foxml:digitalObject/foxml:datastream[@ID='DCQ']/foxml:datastreamVersion[last()]/foxml:xmlContent/dc/dcterms:identifier") {
		if n != nil {
			if txt := n.InnerText(); strings.HasPrefix(txt, "http://hdl.handle.net") {
	...
	if val = ""; err2 != nil {
		...
		ds := expr.Evaluate(xmlquery.CreateXPathNavigator(doc)).(float64)
		val = fmt.Sprintf("%.0f", ds)
		fmt.Printf("# LICENSE datasreams: %s\n", val)
	}
	csv = append(csv, val)

	// DCQ MD5
	val = ""
	for i, n := range xmlquery.Find(doc, "/foxml:digitalObject/foxml:datastream[starts-with(@ID,'DCQ')]/foxml:datastreamVersion/foxml:contentDigest/@DIGEST") {
		if n != nil {
			if txt := n.InnerText(); txt != "" {
				if val != "" {
					val = val + "|"
	...
	// DS MD5
	val = ""
	for i, n := range xmlquery.Find(doc, "/foxml:digitalObject/foxml:datastream[starts-with(@ID,'DS')]/foxml:datastreamVersion/foxml:contentDigest/@DIGEST") {
		if n != nil {
			if txt := n.InnerText(); txt != "" {
				if val != "" {
					val = val + "|"
